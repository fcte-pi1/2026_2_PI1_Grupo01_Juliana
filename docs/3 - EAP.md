...
| 3.2.1 | Sensores infravermelhos de paredes | Sensores para detecção de paredes à esquerda, à direita e à frente do robô. | Previsão do TAP: quatro sensores, sendo dois laterais e dois dianteiros; referências preliminares HW-201 e módulo identificado como LM393. | Detecção a validar em células de 18 cm, paredes de 5 cm de altura e 1,2 cm de espessura, brancas com topo vermelho e piso preto (RF1). |
...
| 3.5 | **Placa e interconexões** | Base física e elétrica de integração dos componentes embarcados. | PCB customizada, conectores, cabeamento e interfaces de alimentação. | Disposição compacta, fixação segura e acesso para manutenção em conjunto com Estrutura (RNF3 e RNF4). |
| 3.5.1 | Placa de circuito impresso (PCB) | Placa própria com posicionamento dos componentes e roteamento das conexões elétricas. | Dimensões máximas de 12 cm × 12 cm; FR4 dupla face como previsão do TAP; trilhas e disposição dimensionadas para sinais e correntes do circuito. | Compatibilidade com os pontos de fixação e com o envelope do robô; circuitos conectados conforme o esquemático (RNF3 e RNF4). |
| 3.5.2 | Conectores e cabeamento | Interligações entre placa, sensores, motores e fonte de energia. | Conectores padronizados, polaridade e pinagem identificadas; JST de quatro vias como referência preliminar para os motores. | Conexões firmes e acessíveis, com prevenção de inversão e sem mau contato durante o movimento (RNF1, RNF3 e RNF7). |
| 3.5.3 | Interface de alimentação da eletrônica | Conexões de entrada e distribuição local das tensões fornecidas por Energia aos componentes eletrônicos. | Barramentos compatíveis com lógica, sensores e drivers; desacoplamento local e organização das conexões para reduzir interferências. | Tensões nos componentes dentro das faixas especificadas, inclusive no acionamento dos motores; regulação, fonte e proteções de potência definidas por Energia (RNF1 e RNF2). |
